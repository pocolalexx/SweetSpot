...
app.post('/api/auth/sign-in', (req, res) => {
    ...
    if (consumeRate(`sign-in:${ip}`, 6, 10 * 60000)) {
        return res.status(429).json({ message: 'Prea multe încercări. Încearcă mai târziu.' });
    }
    const { email = '', password = '', confirmPassword = '', website = '', startedAt = 0, } = req.body;
...
app.post('/api/auth/log-in', (req, res) => {
    const ip = clientIp(req);
    if (consumeRate(`log-in:${ip}`, 10, 10 * 60000)) {
        return res.status(429).json({ message: 'Prea multe încercări. Încearcă mai târziu.' });
    }
    const { email = '', password = '', confirmPassword = '', website = '', startedAt = 0, } = req.body;
    if (typeof website === 'string' && website.trim().length > 0) {
...
app.post('/api/loyalty/scan', (req, res) => {
    const ip = clientIp(req);
    if (consumeRate(`scan:${ip}`, 20, 5 * 60000)) {
